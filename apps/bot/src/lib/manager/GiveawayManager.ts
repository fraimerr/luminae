...
export class GiveawayManager {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        giveaway.description +
          `\n\n` +
          `${Constants.emojis.clock} Ends: <t:${Math.floor(giveaway.endAt! / 1000)}:R>` +
          `\n` +
          `${Constants.emojis.person} Host: <@${interaction.user.id}>`,
      )
      .setFooter({
        text: `${giveaway.winners === 1 ? "1 winner" : `${giveaway.winners} winners`}`,
      })
  ...
  private static async endGiveaway(messageId: string) {
    ...
    if (channel) {
      ...
      if (winners.length > 0) {
        await message.reply({
          content: `${Constants.emojis.confetti} | ${winners.map((w) => `<@${w}>`)}`,
          embeds: [
            new EmbedBuilder()
              .setColor(Constants.primaryColor)
              .setDescription(`Congratulations! You won **${giveaway.title}**!`),
          ],
          components: [
            new ActionRowBuilder<ButtonBuilder>().addComponents(
              new ButtonBuilder().setLabel("Go to Giveaway").setURL(message.url).setStyle(ButtonStyle.Link),
            ),
          ],
        });
      }
  ...
  private static getWinners(giveaway: Giveaways, winnersCount?: number) {
    const validEntries = giveaway.entries.filter((e, i, a) => a.indexOf(e) === i);
  ...
  private static createEndEmbed(giveaway: Giveaways, winners: string[]): EmbedBuilder {
  ...
  private static async getChannel(channelId: string): Promise<TextChannel | null> {
    try {
