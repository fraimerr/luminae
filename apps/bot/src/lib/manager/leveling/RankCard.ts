import { SKRSContext2D, Canvas, loadImage, Image, GlobalFonts } from "@napi-rs/canvas";
...
export class RankCard {
  ...
  public static async getCard(member: GuildMember, levelData: Leveling): Promise<AttachmentBuilder[]> {
    ...
    this.roundRect(ctx, 0, 0, this.CARD_WIDTH, this.CARD_HEIGHT, this.CARD_BORDER_RADIUS);

    ctx.clip();

    GlobalFonts.registerFromPath(path.join(__dirname, "../../../../assets/fonts/FeelinTeachy.ttf"), "FeelinTeachy");
  ...
  private static async extractDominantColor(img: Image): Promise<string> {
    ...
    const samples = [];
    for (let i = 0; i < imageData.length; i += 4 * Math.floor(imageData.length / (4 * 20))) {
  ...
  private static async drawUserBackground(member: GuildMember, ctx: SKRSContext2D): Promise<void> {
    const imgURLOptions: ImageURLOptions = {
      forceStatic: true,
      size: 1024,
      extension: "png",
    ...
    try {
      ...
      ctx.drawImage(background, sx, sy, sWidth, sHeight, 0, 0, this.CARD_WIDTH, this.CARD_HEIGHT);
  ...
  private static async drawAvatar(ctx: SKRSContext2D, avatarImage: Image, accentColor: string): Promise<void> {
    try {
      const centerX = 140;
      ...
      ctx.drawImage(avatarImage, posX, posY, this.AVATAR_SIZE, this.AVATAR_SIZE);
