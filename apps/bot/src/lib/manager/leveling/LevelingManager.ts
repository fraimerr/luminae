...
import { getLevelData, getOrCreateLevelData } from "~/root/database/services/leveling";
import { replaceVariables } from "~/util/replaceVariables";

export class LevelingManager {
  public static async levelUp(message: Message, member: GuildMember, levelData: Leveling, config: LevelingConfig) {
    if (config.channelId) {
      let content = config.message || "{user.mention} has leveled up to **Level {level}**!";
  ...
  public static getRandomXp(min?: number, max?: number) {
    return Math.round(Math.random() * ((max ?? 25) - (min ?? 15))) + (min ?? 15);
  }
}
