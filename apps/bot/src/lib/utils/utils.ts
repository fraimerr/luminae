...
export const getRole = (
	...
	return guild.roles.cache.find(
		(role) =>
			role.id === arg.replaceAll(/\D/g, "") ||
			role.name === arg ||
			role.name.toLowerCase() === arg.toLowerCase()
	);
};
...
export const getChannel = <T extends GuildBasedChannel["type"]>(
	...
	| (T extends GuildBasedChannel["type"]
...
export const getMentionable = (
	{ guild }: { guild: Guild | null },
	arg?: string
): GuildMember | Role | undefined => {
	return getRole({ guild }, arg) ?? getMember({ guild }, arg);
};

export const trim = <T extends string | null | undefined>(
	str: T,
	max: number
): T => {
	if (!str) return str;
...
export const validateColor = (embedColor: unknown): number | false => {
	try {
		return resolveColor(
			toTitleCase(embedColor as string).replace(/ +/, "") as ColorResolvable
		);
	} catch {
		return false;
	}
...
export const authorOrUser = (
	messageOrInteraction: BaseInteraction | Message
) => {
	return "user" in messageOrInteraction
...
>(
	...
	try {
		...
		return {
			error: null,
			result: callback(...args) as T extends (...args: A) => infer R
				? R
				: never,
		};
	} catch (error_) {
...
>(
	...
	components
		...
		.find(
			(component) =>
				"custom_id" in component.data && component.data.custom_id === customId
		) as T | undefined;

export const collectorFilter = (
	incoming: BaseInteraction | Message,
	original: BaseInteraction | Message
...
export const handleReply = (
	...
	options: Omit<
		InteractionReplyOptions & MessageReplyOptions & InteractionEditReplyOptions,
		"flags"
	>,
	ephemeral?: boolean
) => {
	if (messageOrInteraction instanceof Message) {
		...
		return messageOrInteraction.reply(
			ephemeral ? { ...options, flags: MessageFlags.Ephemeral } : options
		);
	}
};
