...
export const syncCommands = async (client: Client<true>) => {
  ...
  Logger.customLog("DEBUG", `Found ${arrayOfChatInputCommands.length} chat input commands`);
  ...
  Logger.customLog("DEBUG", `Found ${arrayOfContextmenuCommands.length} contextmenu commands`);

  const arrayOfGlobalCommands = [...arrayOfChatInputCommands, ...arrayOfContextmenuCommands];

  Logger.customLog("DEBUG", `Found ${arrayOfGlobalCommands.length} global commands`);
  ...
  Logger.customLog("DEBUG", `Found ${arrayOfOwnerCommands.length} owner commands`);
  ...
  Logger.customLog("DEBUG", `Found ${newGlobalCommands.length} new global commands to add`);
  ...
  Logger.customLog("DEBUG", `Found ${newOwnerOnlyCommands.length} new owner-only commands to add`);
  ...
  Logger.customLog("DEBUG", `Found ${deletedCommands.length} commands to delete`);
  ...
  for (const command of allCommands) {
    ...
    if (
      ...
      Logger.customLog("DEBUG", `Updating options for command ${blue(newCommand.name)}`);
      modified = true;
    }
