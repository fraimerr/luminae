...
export default async function giveawayCreate(
  ...
  if (duration === undefined) {
    return interaction.reply({
      content: "The duration provided is invalid. The correct format is `1d 2h 3m 4s`",
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (duration < ms("10s")) {
    return interaction.reply({
      content: "The duration provided is too short. The minimum duration is 1 minute",
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  const buttons1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId("add-requirements").setLabel("Add Req.").setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId("add-multipliers").setLabel("Add Mult.").setStyle(ButtonStyle.Secondary),
  );
  const buttons2 = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`exit-${interaction.user.id}`).setLabel("Cancel").setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId("start-giveaway").setLabel("Start").setStyle(ButtonStyle.Success),
  );
  ...
  collector.on("collect", async (initialInt) => {
    if (initialInt.customId === "add-requirements") {
      ...
      const requirementRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
          ...
          if (i.customId === "requirement-select") {
            ...
            if (requirementType === "have_role") {
              ...
              const roleCollector = roleReply.createMessageComponentCollector({
                ...
                filter: (i) => i.customId === "required-role-select" && i.user.id === interaction.user.id,
              });
              ...
              const requiredMessagesCollector = i.channel?.createMessageCollector({
                time: 30_000,
              ...
              const roleCollector = roleReply.createMessageComponentCollector({
                time: 1000 * 60 * 5,
                componentType: ComponentType.RoleSelect,
                filter: (i) => i.customId === "required-not-role-select" && i.user.id === interaction.user.id,
              });
              ...
              levelCollector?.on("collect", async (msg) => {
                if (isNaN(parseInt(msg.content)) || parseInt(msg.content) <= 0) {
                  await msg.reply({
      ...
      const multiplierRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
        new StringSelectMenuBuilder()
          ...
          if (i.customId === "multiplier-select") {
            ...
            if (multiplierType === "role_multiplier") {
              ...
              const roleCollector = roleReply.createMessageComponentCollector({
                time: 1000 * 60 * 5,
                componentType: ComponentType.RoleSelect,
                filter: (i) => i.customId === "multiplier-role-select" && i.user.id === interaction.user.id,
              });
                ...
                const entriesEmbed = new EmbedBuilder()
                  ...
                  .setDescription(
                    `How many extra entries should users with <@&${
                      ri.values[0]
                    }> get?\n\n- Enter a number <t:${Math.floor(Date.now() / 1000 + 30)}:R>`,
                  );
