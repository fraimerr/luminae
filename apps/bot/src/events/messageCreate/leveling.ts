...
export default async function (message: Message) {
  ...
  const { levelData, levelUp } = await LevelingManager.addXp(userId, guildId, xp);

  if (!levelUp) return;

  if (levelingConfig.announce === false) return;

  const channel = message.guild.channels.cache.get(levelingConfig.channelId!) as TextChannel;

  if (channel) {
    await channel.send({
      content: replaceVariables(levelingConfig.message!, message.member!, levelData, true),
    });
  } else {
