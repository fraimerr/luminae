...
export default async function (message: Message) {
  ...
  const command = availableCommands.get(cmd) ?? availableCommands.find((c) => c.aliases.includes(cmd));
  ...
  if (command) {
    ...
    try {
      const memberPerms = message.channel.permissionsFor(message.guild.members.me as GuildMember);
    ...
    if (chatInputCommand && !chatInputCommand.ownerOnly) {
      await message.reply(`Hey! It's a slash only command. Use \`/${cmd}\` instead.`);
    }
  }
}
