...
export const handler = (client: Client) => {
	const folders = readdirSync("src/commands/chatInputCommands/").filter(
		(file) => lstatSync(`src/commands/chatInputCommands/${file}`).isDirectory()
	);
	for (const dir of folders) {
		const commandFiles = readdirSync(
			`src/commands/chatInputCommands/${dir}`
		).filter((file) => file.endsWith(".ts"));
		for (const file of commandFiles) {
			const { UserCommand } =
				require(`../commands/chatInputCommands/${dir}/${file}`) as {
			...
			if (command.name) {
				...
				if (command.subCommandsHelp) {
					for (const subCommand of command.subCommandsHelp) {
						...
						subCommand.userChannelPermissions ??=
			...
			delete require.cache[
				require.resolve(`../commands/chatInputCommands/${dir}/${file}`)
			];
		}
	}
};
