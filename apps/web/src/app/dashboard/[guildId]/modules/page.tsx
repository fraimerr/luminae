...
export default function ModulesPage({
  ...
  const { data, isLoading } = useApi<APIV1ModulesResponse>(`/modules/${guildId}`, true);
  const [updatingModules, setUpdatingModules] = useState<string[]>([]);

  const { mutate } = useMutation({
    mutationFn: async (moduleId: string) => {
      setUpdatingModules((prev) => [...prev, moduleId]);
      const enabled = !data?.data?.some((m) => m.name === moduleId && m.enabled);

      const response = await fetch(`http://localhost:5000/v1/modules/${guildId}/${moduleId}`, {
        method: "POST",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Enable or disable bot modules</p>
      </div>

      <div className="grid gap-4 md:grid-cols-3">
        {modules.map((module) => {
          const isEnabled = data?.data?.some((m) => m.name === module.id && m.enabled);
            ...
            <Card key={module.id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch checked={isEnabled} onCheckedChange={() => toggleModule(module.id)} disabled={isUpdating} />
                  </div>
                </div>
              ...
              <CardContent>
                <Button variant="outline" size="sm" className="w-full" asChild>
                  <Link href={`/dashboard/${guildId}/${module.id}`}>Configure</Link>
                </Button>
              </CardContent>
            </Card>
          );
