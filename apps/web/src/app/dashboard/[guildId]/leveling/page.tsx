...
const formSchema = z.object({
  ...
  // TODO: Add role rewards and XP multiplier schemas
});

type FormValues = z.infer<typeof formSchema>;
type PageParams = { guildId: string };

export default function LevelingPage({ params }: { params: PageParams | Promise<PageParams> }) {
  const resolvedParams = React.use(params as any) as PageParams;
  const guildId = resolvedParams.guildId;
  
  const queryClient = useQueryClient();
  const { data: configData, isLoading } = useApi<{ data: FormValues }>(
    `/modules/${guildId}/leveling/config`,
    true
  );
  const { data: channels } = useApi<{ data: Array<{ id: string; name: string }> }>(
  ...
  const { mutate } = useMutation({
    mutationFn: async (values: FormValues) => {
      const response = await fetch(
        `http://localhost:5000/v1/modules/${guildId}/leveling/config`,
        {
          method: "POST",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`/modules/${guildId}/leveling/config`],
      });
    },
  });
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Customize the message sent when users level up. Available
                        variables: {"{user.mention}"}, {"{level}"}
                      </FormDescription>
                      <FormControl>
                        <Textarea
